...
exports.handler = async (event) => {
  ...
  try {
    ...
    for (let waveStart = 0; waveStart < batches.length; waveStart += 2) {
      ...
      const wavePromises = waveBatches.map(async (batch, relativeIdx) => {
        ...
        const destinations = batch
          .map(p => { const lat = p.geometry?.location?.lat ?? p.lat ?? p.latitude; const lng = p.geometry?.location?.lng ?? p.lng ?? p.longitude; return `${lat},${lng}`; })
          .join('|');
    ...
    // For candidates not enriched by DM, estimate walk/drive time from distanceMiles
    const enrichedIds = new Set(enriched.map(r => r.place_id));
    const unenriched = sortedCandidates.slice(maxDmBudget).map(place => {
      const miles = place.distanceMiles ?? null;
      const estWalk = miles != null ? Math.round(miles * 20) : null;
      const estDrive = miles != null ? Math.round(miles * 3 + 5) : null;
      return {
        ...place,
        walkMinutes: place.walkMinutes ?? estWalk,
        driveMinutes: place.driveMinutes ?? estDrive,
        transitMinutes: place.transitMinutes ?? estWalk,
      };
    });
    const allEnriched = [...enriched, ...unenriched];
    console.log(`Estimated times for ${unenriched.length} unenriched entries`);

    console.log('=== PERFORMANCE ===');
    console.log('distance_matrix_ms:', dmTime);
    console.log('total_ms:', totalTime);
    console.log('candidates_enriched:', allEnriched.length);
    console.log('===================');
    ...
    return {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        enrichedCandidates: allEnriched,
        performance: {
